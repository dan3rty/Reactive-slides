...
function AddBar() {
	...
	function useHandleAddUp(event: MouseEvent) {
		...
		const objectToAdd= currentBlock === 'rectangle' || currentBlock === 'triangle' || currentBlock === 'oval' ? {
			blockType: Type.BlockType.PRIMITIVE,
			id: Math.random().toString(16).slice(2),
			primitiveType: currentBlock === 'rectangle'? Type.Primitives.RECT : currentBlock === 'triangle'? Type.Primitives.TRIANGLE : Type.Primitives.CIRCLE,
			color: {
	...
	function handleAddDown(event: MouseEvent) {
		...
		if (rect.x > event.x || rect.x + rect.width < event.x || rect.y > event.y || rect.y + rect.height < event.y) {
			return
		}
	...
	return (
		<div className={styles.addBar}>
			<Button style='light' size='big' icon={TextIcon} text='text' onClick={() => {
				document.addEventListener('mousedown', handleAddDown)
				currentBlock = 'text'
			}}/>
			<Button style='light' size='big' icon={TableIcon} text='table' />
			<Button style='light' size='big' icon={TriangleIcon} text='triangle' onClick={() => {
				document.addEventListener('mousedown', handleAddDown)
				currentBlock = 'triangle'
			}}/>
			<Button style='light' size='big' icon={BulletListIcon} text='bullet list' />
			{!imagePathInputOpened && (<Button style='light' size='big' icon={ImageIcon} text='image' onClick={() => {
				setImagePathInputOpened(true)
				document.addEventListener('mousedown', handleDownInInput)
			}}/>)}
			{imagePathInputOpened && (<input className={styles.imagePathInput} ref={imagePathInputRef} placeholder={'URL to image'} onChange={(e) => {
				setImagePathInputOpened(false)
				currentBlock = 'image'
				imagePathInput = e.target.value
				document.addEventListener('mousedown', handleAddDown)
			}}/>)}
			<Button style='light' size='big' icon={SquareIcon} text='rectangle' onClick={() => {
				document.addEventListener('mousedown', handleAddDown)
			...
			<Button style='light' size='big' icon={OvalIcon} text='oval' onClick={() => {
				document.addEventListener('mousedown', handleAddDown)
				currentBlock = 'oval'
			}}/>
		</div>
